...
for country in pycountry.countries:
    emoji_flag_mapping[country.name] = country_to_emoji(country.name)
emoji_flag_mapping.update({name: emoji for name, emoji in custom_country_emoji.items()})
    ...
    for node in net_graph.nodes:
        if node["id"] in flat_edge_df["Source"].values:
            ...
            if target_col == "Nationality":
                emoji_flag = emoji_flag_mapping.get(node["id"], "")
                node["label"] = f"{emoji_flag} {node['id']}"
                node["color"] = "#00fa9a"
                node["shape"] = "ellipse"
